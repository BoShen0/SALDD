...
function results = OnlineOptimiationTG_SRconvergence(TransferData, YLabels, T, options)
...
for iter = 1:T
weights = theta;
theta = theta * sqrt(length(theta))/max(norm(theta),sqrt(length(theta)));
...
%         ThetaTemp(ThetaTemp<0) = 0; % non-negative weights
        ...
        SparsityRecovery(iter) =  max(SparseRecoveryIndex,Sparsity);
...
weights = weights * sqrt(length(weights))/max(norm(weights),sqrt(length(weights)));
Score = [];
Score = - TransferData(D,:) * theta;
% [maxValueTheta,maxIndex] = max(Score);
maxIndex = randi([1 length(Score)],1);
maxValueTheta = Score(maxIndex);
...
HumanFeedOutput(iter,3) = maxValueTheta;
HumanFeedOutput(iter,4) = - TransferData(tempIndex,:) * weights;
